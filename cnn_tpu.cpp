...
m4D P_3(BATCH_SIZE, m3D(15, m2D(15, m1D(32, 0)))); //1 layer padding
m4D L_4(BATCH_SIZE, m3D(13, m2D(13, m1D(64, 0))));
m4D L_5(BATCH_SIZE, m3D(11, m2D(11, m1D(64, 0))));
m4D P_6(BATCH_SIZE, m3D(7, m2D(7, m1D(64, 0)))); //1 layer padding
m4D L_7(BATCH_SIZE, m3D(5, m2D(5, m1D(256, 0))));
m4D L_8(BATCH_SIZE, m3D(3, m2D(3, m1D(256, 0))));
m4D P_9(BATCH_SIZE, m3D(1, m2D(1, m1D(256, 0)))); //1 layer padding
...
void test() {
    ...
    for (int i = 0; i < 1000 / BATCH_SIZE; i++) {
        m3D batch = m3D(X_test.begin() + i * BATCH_SIZE, X_test.begin() + i * BATCH_SIZE + BATCH_SIZE);
        vector<int> labels = vector<int>(Y_test.begin() + i * BATCH_SIZE, Y_test.begin() + i * BATCH_SIZE + BATCH_SIZE);
        feed_forward(batch);
        for (int j = 0; j < BATCH_SIZE; j++) {
            if (max_element(L_11[j].begin(), L_11[j].end()) - L_11[j].begin() == labels[j]) {
                test_accuracy += 0.1;
            }
        }
    }
    cout << "Validation accuracy: " << test_accuracy << "%\n";
}
